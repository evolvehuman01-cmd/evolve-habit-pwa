...
// CoachDashboard.jsx
// Route: /coach
// Auth guard: Google OAuth email must match VITE_COACH_EMAIL
// ─────────────────────────────────────────────────────────────────
...
async function apiPost(body) {
  const res = await fetch(DATA_SCRIPT, {
    method: 'POST',
...
function OverviewTab({ logs, targets }) {
  ...
  const logsByDate = {};
  logs.forEach(l => { if (l.date) logsByDate[l.date] = l; });
...
function HabitAverages({ logs, targets }) {
  const HABITS = [
    { key: 'water',      label: 'Water (L)',     unit: 'L'  },
    { key: 'sleep',      label: 'Sleep (hrs)',   unit: 'hr' },
    { key: 'steps',      label: 'Steps',         unit: ''   },
    { key: 'protein',    label: 'Protein (g)',   unit: 'g'  },
    { key: 'mood',       label: 'Mood (1–10)',   unit: ''   },
    { key: 'energy',     label: 'Energy (1–10)', unit: ''   },
    { key: 'digestion',  label: 'Digestion',     unit: ''   },
...
function TargetEditor({ client, onSaved, onCancel }) {
  ...
  const TARGET_FIELDS = [
    { key: 'water',     label: 'Daily water target (L)',     type: 'number', step: '0.1' },
    { key: 'sleep',     label: 'Sleep target (hrs)',         type: 'number', step: '0.5' },
    { key: 'steps',     label: 'Step target',                type: 'number', step: '500' },
    { key: 'protein',   label: 'Protein target (g)',         type: 'number', step: '5'   },
    { key: 'mood',      label: 'Mood baseline (1–10)',       type: 'number', step: '1'   },
    { key: 'energy',    label: 'Energy baseline (1–10)',     type: 'number', step: '1'   },
    { key: 'digestion', label: 'Digestion target (1–5)',     type: 'number', step: '1'   },
    { key: 'mobility',  label: 'Mobility target (1–5)',      type: 'number', step: '1'   },
  ];
